...
const StatCard = ({ label, value, color }) => (
  <div className={`bg-white rounded-2xl shadow border border-gray-100 p-5 flex flex-col gap-1`}>
  ...
  return (
    <div className="max-w-5xl mx-auto mt-8 px-4">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <StatCard label="Total Users" value={users.length} color="text-indigo-600" />
        <StatCard label="Verified" value={verified} color="text-green-600" />
        <StatCard label="Unverified" value={unverified} color="text-red-500" />
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-indigo-50 text-indigo-700 uppercase text-xs">
              <tr>
                {["User", "Email", "First Name", "Last Name", "Verified", "Actions"].map((h) => (
                  ...
                  <tr key={u.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3">
                      <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                        u.emailVerified ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"
                      }`}>
