...
const icons = {
  firstName: (
    <svg className="h-4 w-4 text-purple-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
  ),
  lastName: (
    <svg className="h-4 w-4 text-purple-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
  ),
  username: (
    <svg className="h-4 w-4 text-purple-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
  ),
  email: (
    <svg className="h-4 w-4 text-purple-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
  ),
  phoneNumber: (
    ...
    <div className="max-w-lg mx-auto mt-8">
      <div className="bg-white shadow-2xl rounded-2xl overflow-hidden border border-gray-200">
        ...
        <div className="px-8 py-6">
          {status && (
            <p className={`mb-4 text-center text-sm font-medium ${status.includes("failed") ? "text-red-600" : "text-green-600"}`}>
              {status}
            </p>
          )}
          ...
          {editing ? (
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="button"
                  onClick={() => { setEditing(false); setEditData({ ...user, password: "" }); }}
              ...
              <div className="space-y-3">
                {fields.map(({ key, label }) => (
                  <div key={key} className="flex items-center gap-3 py-2 border-b border-gray-100 last:border-0">
                    ...
                    <span className="text-xs font-semibold text-gray-400 w-24 flex-shrink-0 uppercase tracking-wide">{label}</span>
                    <span className="text-gray-800 text-sm">{user[key] || "—"}</span>
                  </div>
                ))}
              </div>
              <button
